...
public interface MoviesRepository extends JpaRepository<Movies, String> {
    ...
    public List<Object[]> getMovieGenreWithSubtotal();
	
	public List<Movies> findTop10ByOrderByRuntimeMinutesDesc();

	@Query("SELECT m.tconst, m.primaryTitle, m.genres, r.averageRating FROM Movies m JOIN Ratings r ON m.tconst = r.tconst WHERE r.averageRating > 6.0 order by r.averageRating Asc")
	public List<Object[]> getMoviesWithAtLeastAverageRation();
	...
	@Query("UPDATE Movies SET runtimeMinutes = "
			...
			+ "END")
	public void incrementRuntimeMinutesAsPerGenre();
}
